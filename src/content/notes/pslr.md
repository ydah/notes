...
Lexは通常、規則の宣言順とlongest matchによってこれらを暗黙に解決する。そのためgrammarやsub-languageを追加したとき、tokenizationが警告なく変わり得る。PSLRはautolength conflictだけを既定のlongest matchで解決する。それ以外の未解決conflictは、parser conflictと同じように報告する。
...
traditionalな三演算子はkeywordとidentifier、`0`とoctal literalなどを表現する。`-s`は複数行commentの終端を簡潔な正規表現で表す。`-<`や`<<`は、特定の文法contextで長いtokenを無効化する場合に使う。

生成系のresolverはscanner FSAを走査し、無限に存在し得る具体的な入力を有限個のscanner conflict profileへまとめる。各profileをlexical precedenceで解決する。未解決conflictと、どのstateにも作用しないuseless ruleは報告する。
...
`scanner_accepts`は$acc(s_p)$によるfilterとidentity conflictの解決結果をstateごとに焼き込む。`length_precedences`はlongest、shortest、token優先をtoken対の真偽値として表す。runtimeでは複雑なprecedence関数を評価せず、table lookupだけで判断できる。
...
受理stateへ到達するたびに`scanner_accepts`を引く。現在のbest matchが新しい候補に負ける場合だけ更新する。FSAが遷移できなくなるか入力が尽きたら、best matchの終端まで入力を消費してtokenを返す。入力が空ならend tokenを返す。

どの位置のmatchを採用するかは、最後まで候補を比較しなければ確定しない。そのため実装には入力のbufferingと、best matchより先まで読んだ文字の巻き戻しが必要になる。

## lexical tie

pseudo-scannerの素のfilterには、reserved keywordをidentifierとして返し得る問題がある。たとえばkeyword `int`が受理不能で`ID`だけが受理可能なstateでは、入力`int`を`ID`として選ぶ。しかしreserved keywordを持つ言語では、`int`を常にkeywordとして認識する。そのcontextで使えなければsyntax errorにすべきである。
...
この宣言により、`ID`が認識されるcontextでは、tieされたkeywordも$acc(s_p)$へ加わる。`int`はkeywordとして選ばれる。そのkeywordをparserが受理できなければsyntax errorになる。

tie関係は反射的・対称的・推移的に扱われる。symbol setへ指定した場合は、その中で実際にconflictするtoken対だけをtieする。どのtokenをtieすべきかは言語の意図に依存する。generatorは、片方だけが受理されるstateを持つ競合対をtie候補として報告する。ユーザーは`%lex-tie`で採用するか、`%lex-no-tie`で意図的に採用しないことを示す。

tieは$acc(s_p)$を広げ、新しいpseudo-scanner conflictを作り得る。そのため、tieの評価はconflictの発見と解決より前に行う。

## IELRとPSLR向けstate compatibility

PSLRでは、parser stateがscannerの左文脈でもある。このためstate mergingはparser action以外にも影響する。二つのstateをmergeすると$acc(s_p)$もunionされるため、parser actionが正しくてもpseudo-scannerの選択tokenが変わる場合がある。

[[lalr-parser|LALR(1)]]は同じLR(0) coreを持つstateを広くmergeする。一方、[[canonical-lr-parser|Canonical LR(1)]]は文脈を細かく保つが、実用grammarではtableが大きくなる。DennyとMalloyの実験では、C++ grammarのstate数はLALRが822、Canonical LRが9849、[[ielr|IELR]]が836であった。
...
実装では無限個の文字列を比較せず、scanner FSAから事前に要約したpairwise conflictを使う。未解決conflictはmergeを妨げない。これだけを理由にstate splitすると、作成途中のgrammarでtableがCanonical LR並みに膨らむためである。全stateに存在するlayout tokenもsplitで除去できないため、split-stableとしてannotation対象から外せる。

このPSLR向けcompatibility testは、state mergingによって正しい入力のtokenizationまで変わる問題を防ぐ。PSLR実装の中心となる追加部分である。
...
これにより、壊れた入力でもscannerは決定的にtokenを返す。parserの[[syntax-error|syntax error]]処理へ制御を渡せる。tieによって候補へ入ったtokenなら言語仕様に基づく選択であり、state mergingによる候補なら近い構文contextに基づく推測になる。

### parser側のLAC

LR parserが不正なlookaheadをすぐに検出できない原因は、state merging、default reduction、[[nonassoc|%nonassoc]]によるerror actionである。不正なlookaheadの検出前に余分なReduceと[[semantic-action|semantic action]]を実行し、expected token listや[[error-recovery|error recovery]]の開始contextも不正確になり得る。
...
LACはPSLRのコアではない。正しい入力のtoken選択とparser actionは、pseudo-scannerとPSLR向けIELR拡張が担保する。LACが改善する範囲と歴史は[[lac-and-pslr|LACとPSLRの関係]]で整理した。
...
`pseudo_scan`がlayout tokenを認識すると、parserへ返さず破棄してscanを再開する。複数のlayout tokenを宣言できる。line commentにはlongest match、multiline commentにはshortest matchという異なる規則を指定できる。
...
統合仕様には独立したlexer action欄がないため、semantic valueの構築には`%token-action`を使う。連続したlayout lexemeは蓄積し、次の非layout tokenのactionから参照できるようにする。入力末尾のlayoutはend tokenのactionから参照できる。これにより、commentを保持するcode transformationにも対応する。
...
runtimeでは、parserが現在stateを渡して`pseudo_scan`を呼ぶ。pseudo-scannerはFSAで文字列を読み、$acc(s_p)$相当の`scanner_accepts`で構文contextに合う候補へ絞る。`length_precedences`で一つを選ぶ。matchがなければfallbackとcharacter tokenがparserのerror処理へ接続する。LACを有効にすれば、不正入力時の診断と回復も補正する。

PSLRはscannerless parserではない。scannerを残し、その判断をparser stateで駆動する。字句解析と構文解析の境界を宣言的に接続する方式である。

## Lramaとの関係

[[lrama|Lrama]]でPSLRを実現する場合、既存のIELR実装がstate splittingの土台になる。scanner FSA、lexical precedenceとtie、`scanner_accepts`は別途必要になる。`pseudo_scan`とPSLR向けstate compatibilityも必要である。
