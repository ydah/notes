...
GNU Bisonでは文法に`%glr-parser`を指定する。Menhirでは`--GLR`バックエンドを使い、生成されたOCamlコードを`MenhirGLR`ライブラリと組み合わせる。分岐中の[[semantic-action|semantic action]]もReduce時に実行されるため、候補ごとの処理が混在しうる。副作用は避けるのが安全。

GLRは「どの解釈が正しいか」を自動では決めない。複数の解析結果を残すか、merge functionや優先順位で1つにまとめるかは文法側で指定する。
