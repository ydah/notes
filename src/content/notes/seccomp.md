...
seccompは、プロセスが発行できるシステムコールをLinuxカーネル側で制限する仕組み。SECure COMPutingの略。使っていないシステムコールを禁止し、プロセスからカーネルへの攻撃面を減らす。

seccompが制限するのはシステムコールの入口であり、ファイルシステムやネットワークなどのポリシー全体ではない。seccompだけでは完全なサンドボックスにならない。必要に応じて他の権限制御やLSMなどと組み合わせる。
...
filter modeでは、BPFプログラムがシステムコールごとに実行される。BPFプログラムは`struct seccomp_data`からシステムコール番号、アーキテクチャ、命令ポインタ、最大6個の引数を読む。その結果に応じて、カーネルが取るべきactionを返す。
...
filter modeを無特権プロセスから有効にする場合、先に`PR_SET_NO_NEW_PRIVS`を設定する。設定しなければ、悪意のあるfilterを付けたままset-user-IDプログラムを`execve`できてしまう。`CAP_SYS_ADMIN`を持つ場合はこの条件が別扱いになる。
...
filterは重ねてインストールできる。複数のfilterがあるときはすべて評価され、最も優先度の高いactionが採用される。後から追加したfilterのデータが使われる場合もある。単純に「最後のfilterが結果を上書きする」とは考えられない。
...
seccompの主な目的は、アプリケーションに不要なカーネル機能を露出させないこと。プロセスがすでに開いているファイルディスクリプタや、システムコール以外で完結するカーネル機能まで一律に消す仕組みではない。

seccomp filterのBPFはポインタを追跡して参照できない。システムコール引数のレジスタ値は検査できるが、ポインタが指すユーザー空間メモリの内容は読めない。この制約によって、カーネル内のfilter評価を単純に保つ。
