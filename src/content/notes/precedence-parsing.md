...
[[shift-reduce-parsing|shift-reduce構文解析]]の一種だが、一般のLRパーサーが文法全体から構文解析表を作るのに対し、順位構文解析は演算子間の関係を直接使って式を処理する。GNU Bisonの[[left-right|%left]]や[[left-right|%right]]は、一般のLR表で発生したshift/reduce conflictを演算子の優先順位・結合方向で解決する仕組みであり、同じ考え方を利用するが、順位構文解析そのものとは別の仕組み。
