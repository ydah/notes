...
例えば、Rustの次のコードでは、空のvectorを作った時点では要素型が決まっていない。その後にpushされた文字列から`Vec<&str>`と推論できる。
...
実装では、まだ正確な型が分からない箇所に推論変数を置き、式を調べながら制約を集める。制約を解いて推論変数を具体的な型へ置き換える処理には、unificationを使う。
...
型推論は[[name-resolution|名前解決]]とは異なる。名前解決は識別子を定義へ対応付け、型推論は式や値の型を決める。どちらも[[semantic-analysis|意味解析]]の一部。型推論の結果は、型検査で代入可能性や演算子の適用可否を確認するために使う。
