...
導出途中で終端記号と非終端記号が混在する記号列をsentential formと呼ぶ。終端記号だけになったものが、文法の言語に含まれるsentenceになる。

毎回一番左の非終端記号を展開する方法を左端導出、一番右を展開する方法を[[rightmost-derivation|右端導出]]と呼ぶ。[[ll-parser|LLパーサー]]は左端導出、[[lr-parser|LRパーサー]]は右端導出を逆向きに扱う。
