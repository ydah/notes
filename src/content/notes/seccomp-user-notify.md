...
seccomp user notifyは、seccomp filterが特定のシステムコールをユーザー空間のsupervisorへ通知する仕組み。supervisorは、そのシステムコールをどう扱うか返す。filterのactionには`SECCOMP_RET_USER_NOTIF`を使う。

通常のseccompでは、filterがシステムコールをその場で許可・拒否する。user notifyでは、supervisorが対象プロセスの情報を調べたり、代わりに処理したりできる。これにより、filter自身はポインタの指すメモリを読めないという制約を補える。
...
3. listener fdをtargetからsupervisorへ渡す。Unixドメインソケットの`SCM_RIGHTS`などを使える。
...
listener fdは特定のスレッドではなく、filterに紐づく。targetがforkすると、そのfilterを共有するプロセスからの通知も同じfdに届く。

## supervisorの応答

supervisorは、targetのシステムコールを実際には実行せず、成功またはエラーの返り値だけをtargetに返せる。

`struct seccomp_notif_resp`の`val`は成功時の返り値、`error`はエラー番号を表す。`id`は受け取った通知と対応付けるための値。targetのシステムコールをカーネルで続行させる場合は`SECCOMP_USER_NOTIF_FLAG_CONTINUE`を使う。

たとえば、より強い権限を持つコンテナ管理プロセスがsupervisorになる。targetからの`mount`やデバイス操作の要求を受け取り、必要な処理を代行して結果を返す。
...
通知後、supervisorの検査中にtargetの別スレッドや攻撃者がメモリを書き換える可能性がある。この場合、検査した値とカーネルが実際に使う値が異なる。検査対象のデータをsupervisor側へコピーしてから判断するなど、競合を考慮する必要がある。

このため、seccomp user notifyはセキュリティポリシーを実装するための機構ではない。基本的な用途は、特権の低いtargetの処理を、より特権の高いsupervisorが代行すること。`CONTINUE`するなら、別の仕組みかカーネル自身が最終的な安全性を保証する必要がある。
...
通知と応答の構造体は、カーネルのバージョンによって拡張される可能性がある。そのため、supervisorは`SECCOMP_GET_NOTIF_SIZES`で`struct seccomp_notif`などのサイズを確認してからバッファを確保する。
