...
`ε`は長さ0の文字列。入力中の文字やtokenを1つも消費しないことを表す。
...
この文法では、`list`が`item list`として続くか、何も生成せずに終わる。後者の[[production-rule|生成規則]]を空規則と呼ぶ。GNU Bisonでは、空規則を明示するために`%empty`も使える。

`ε`は[[empty-set|空集合]]ではない。空集合は要素を持たない集合であり、`ε`は空の文字列という要素。[[first-set|FIRST集合]]が`ε`を含む記号列は、空文字列へ導出できる。
