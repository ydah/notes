...
Rubyの構文解析器。portable、error-tolerant、maintainableなRuby parserを目指している。C99で実装され、外部依存なしでビルドできる。parser generatorではなく、Rubyの文法を解析するparserの実装そのものである。

PrismはCのlibprismとして利用でき、CRuby向けのextensionも提供する。CRuby 3.3以降には組み込まれており、それより前のRubyではprism gemをインストールして使う。

Prismの解析結果は[[ast|AST]]と診断情報を持つ。ノードにはソース上の位置が関連づけられる。構文エラーによってソースに存在しないノードが必要になる場合は、MissingNodeのようなerror recovery nodeで表現する。
