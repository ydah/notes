...
CSTが文法ファイルの全ての生成規則を一つずつ可視ノードにするとは限らない。Tree-sitterのようにnamed nodeとanonymous nodeを分けたり、hidden ruleやaliasを使ったりする実装もある。
...
[[ast|AST]]とCSTは必ず別の木になるわけではない。rust-analyzerではlosslessな[[syntax-tree|構文木]]の上に、抽象化されたAST APIを重ねる。
