...
rust-analyzerの構文木は、[[green-tree|Green Tree]]をデータ層にする。そこへ[[red-green-tree|Red-Green Tree]]の[[red-tree|Red Tree]]と、その上のAST APIを組み合わせる。構文木とASTが別の木として存在するとは限らない。同じsyntax treeに異なるビューを重ねる設計もある。

構文エラーがあっても、木全体を捨てる必要はない。Roslynは欠落tokenや[[skipped-token|skipped token]]を木に残す。一方、rust-analyzerはparser errorを木の外の配列で管理し、余分な入力を`ERROR`ノードとして木に含める。欠落した必須ノードが常に木に現れるわけではない。構文エラーそのものを[[syntax-error|構文エラー]]として、木とは別の診断情報にする設計もある。
