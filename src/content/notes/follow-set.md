...
文法中の非終端記号の直後に現れうる終端記号を集めた集合。文法全体からパーサー生成時に計算するもので、実際の入力を読んだときの次のtokenそのものではない。[[lookahead|lookahead]]の候補を文法からまとめたもの、と考えると分かりやすい。
...
の形なら、`β`の先頭に現れうる終端記号をFOLLOW(A)へ追加する。`β`が空文字（`ε`）になりうる場合は、FOLLOW(X)もFOLLOW(A)へ追加する。
