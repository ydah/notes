...
入力を左（Left）から右（Right）へ読みながら、左端導出を行う構文解析方式。開始記号から出発し、lookahead tokenを見て次の[[production-rule|生成規則]]を選ぶ。[[lr-parser|LRパーサー]]が[[shift|Shift]]・[[reduce|Reduce]]で入力を畳み込むのに対し、LLパーサーはこれから読む構造を予測して上から下へ展開する。
...
LLは文法から処理の流れを想像しやすく、再帰下降パーサーをデバッグしやすい。ただし、左再帰の除去や左因子分解が必要になる場合がある。LRは左再帰をそのまま扱いやすく、より広い文法を扱える。代わりに、状態・パーサーテーブル・conflictの理解が必要になる。
