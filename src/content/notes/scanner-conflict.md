...
scanner conflictは、同じ入力位置で[[scanner]]が複数のtoken候補を認識し、tokenizationを一意に決められない状態。
...
[[lex|Lex]]や[[flex]]のscannerは、length conflictを最長一致で解決し、identity conflictを規則の記述順で解決する。この規則は単独の言語では便利だが、[[composite-language|composite language]]ではparserの状態によって正しい選択が変わることがある。
...
parser conflictは、同じtoken列に対して[[action-table|ACTION表]]の複数の動作が競合する問題。scanner conflictは、token列を作る前にtokenの切り方が競合する問題。二つは別の段階で起きるが、一つの文法に同時に存在することはある。
